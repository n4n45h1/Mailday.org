...
- Recovery protection: Standard / Strong は Argon2id（64MiB）+ XChaCha20-Poly1305
- PostalMime + DOMPurify: 端末内MIME解析と安全なHTML表示

標準・強力モードでは、R2へ保存される前に受信したraw MIME全体をWorkerのメモリ上で暗号化します。秘密鍵と平文メールをサーバー側へ永続化しません。「暗号化なし」を明示的に選択したInboxは例外です。過去に作成したmd1/md2 Recovery KeyとP-256/P-384 Inboxの復号互換性も維持しています。

新規の標準・強力モードのRecovery Keyはmd3形式で、固定・検証済みのArgon2idパラメータを使って暗号化します。Recovery Key内のKDFパラメータを無制限に信用しないため、細工したキーによる過大なメモリ・CPU消費も防ぎます。
...
4. `npm run deploy`を実行します。`deploy`は先に`npm run build`を実行し、`client/app.js`から`public/app.js`を再生成します。
